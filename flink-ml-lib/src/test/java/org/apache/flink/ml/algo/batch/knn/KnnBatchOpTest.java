...
public class KnnBatchOpTest {
	private BatchOperator <?> getSourceOp(List <Row> rows) {
		...
		Table out =
			MLEnvironmentFactory.getDefault().getStreamTableEnvironment().fromDataStream(dataStream);
	...
	public void testKnnTrainBatchOp() throws Exception {
		...
		MLEnvironmentFactory.getDefault().getStreamTableEnvironment().toDataStream(
			result.getOutput())
			.addSink(
	...
	public void testKnnPipeline() throws Exception {
		...
		Table result = pipe.fit(source).transform(source)[0];
			...
			MLEnvironmentFactory.getDefault().getStreamTableEnvironment().toDataStream(result)
				.addSink(
					new SinkFunction <Row>() {
						@Override
						public void invoke(Row value, Context context) throws Exception {
