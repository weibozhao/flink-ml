...
/**
 * LongMatrix stores a 2 dimension long array, and provides some methods to operate on the array it
 * represents.
 */
public class LongMatrix implements Serializable {
    ...
    public LongMatrix(long[][] matrix) {
        ...
        for (int i = 1; i < rowNum; i++) {
            Preconditions.checkState(
                    matrix[i].length == colNum, "The column numbers are not equal!");
        }
    }
    ...
    public void plusEqual(LongMatrix other) {
        Preconditions.checkArgument(
                other.rowNum == rowNum && other.colNum == colNum,
                "Matrix size not the same! The rowNums are {},{}; the colNums are {},{}.",
                rowNum,
                other.rowNum,
                colNum,
                other.colNum);
    ...
    public long getValue(int i, int j) {
        Preconditions.checkArgument(
                i >= 0 && i < rowNum && j >= 0 && j < colNum, "Index out of bound!");
        return matrix[i][j];
    }
