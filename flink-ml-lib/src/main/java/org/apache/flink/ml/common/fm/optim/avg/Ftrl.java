...
/**
 * An iteration stage that uses the current batch samples to update the model values with Ftrl
 * method.
 */
public class Ftrl extends BaseAvgOptimizer {
    ...
    private void updateOneDim(double grad, double[] modelValues, int idx, int offset, int bias) {
        double sigma =
                (Math.sqrt(modelValues[idx + offset + bias] + grad * grad)
                                - Math.sqrt(modelValues[idx + offset + bias]))
                        / alpha;
        modelValues[idx + 2 * offset + bias] += grad - sigma * modelValues[idx + bias];
        modelValues[idx + offset + bias] += grad * grad;
        if (Math.abs(modelValues[idx + 2 * offset + bias]) <= l1) {
            modelValues[idx + bias] = 0.0;
        } else {
            modelValues[idx + bias] =
                    ((modelValues[idx + 2 * offset + bias] < 0 ? -1 : 1) * l1
                                    - modelValues[idx + 2 * offset + bias])
                            / ((beta + Math.sqrt(modelValues[idx + offset + bias])) / alpha + l2);
        }
    }
}
