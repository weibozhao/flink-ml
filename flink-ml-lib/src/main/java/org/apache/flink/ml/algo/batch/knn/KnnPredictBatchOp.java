...
/**
 * batch map batch operator. you can inherit this class to develop model-based prediction batch operator.
 */
public class KnnPredictBatchOp extends BatchOperator <KnnPredictBatchOp>
	...
	public Table[] transform(Table... inputs) {
		...
		DataType idType = modelSchema.getColumnDataTypes().get(modelSchema.getColumnNames().size() - 1);

		ResolvedSchema outputSchema = getOutputSchema(inputs[0].getResolvedSchema(), getParamMap(), idType);
		...
		Function <List <DataStream <?>>, DataStream <Row>> function =
			dataStreams -> {
				...
				return stream.transform(
					...
					new PredictOperator(new KnnRichFunction(getParamMap(), inputs[0].getResolvedSchema())));
			};
	...
	private static class KnnRichFunction
		 ...
		 */
		private static Tuple2 <List <Object>, List <Object>> extractObject(String json, DataType idType) {
			Map <String, String> deserializedJson;
			try {
				deserializedJson =
					pGson.fromJson(
						json, new TypeReference <Map <String, String>>() {}.getType());
			} catch (Exception e) {
			...
			if (ids == null) {
				...
				map.put(
					"id",
					pGson.fromJson(
						ids, ParameterizedTypeImpl.make(List.class, new Type[] {type}, null)));
			}
		...
		public void loadModel(List <Object> broadcastVar) {
			...
			if (meta.containsKey(KnnTrainParams.FEATURE_COLS.name)) {
				selectedCols = pGson.fromJson ((String) meta.get(KnnTrainParams.FEATURE_COLS.name), String[].class);
			} else {
				vectorCol = pGson.fromJson ((String) meta.get(KnnTrainParams.VECTOR_COL.name), String.class);
			}

			BaseFastDistance distance = pGson.fromJson((String) meta.get(KnnTrainParams.DISTANCE_TYPE.name),
				DistanceType.class).getFastDistance();
			modelData =
				new KnnModelData(
					dictData, distance);
			idType = LogicalTypeDataTypeConverter.toDataType(LogicalTypeParser.parse((String) this.meta.get("idType")));
			modelData.setIdType(idType);
		}
	}

	/**
	 * this operator use mapper to load the model data and do the prediction. if you want to write a prediction
	 * operator, you need implement a special mapper for this operator.
	 */
	private static class PredictOperator extends AbstractUdfStreamOperator <Row, RichMapFunction <Row, Row>>
		implements OneInputStreamOperator <Row, Row> {

		public PredictOperator(
			RichMapFunction <Row, Row> userFunction) {
			super(userFunction);
		}
	...
	public ResolvedSchema getOutputSchema(ResolvedSchema dataSchema, Map <Param <?>, Object> params, DataType idType) {
		String[] reservedCols = (String[]) params.get(KnnPredictParams.RESERVED_COLS);
		reservedCols = (reservedCols == null)
				? dataSchema.getColumnNames().toArray(new String[0]) : reservedCols;
