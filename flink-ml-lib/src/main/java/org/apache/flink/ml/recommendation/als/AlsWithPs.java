 ...
 */
public class AlsWithPs implements Estimator<AlsWithPs, AlsModel>, AlsParams<AlsWithPs> {
    private final Map<Param<?>, Object> paramMap = new HashMap<>();
    private static final Logger LOG = LoggerFactory.getLogger(AlsWithPs.class);
    ...
    private static class GenerateRequest extends AbstractStreamOperator<Tuple3<Integer, Byte, Long>>
        ...
        @Override
        public void processElement(StreamRecord<Tuple2<Integer, Ratings>> streamRecord)
                throws Exception {
            Ratings value = streamRecord.getValue().f1;
    ...
    private static class IterationControllerFunc
        ...
        private final Map<String, Factors> factorsMap = new HashMap<>();
        ...
        public void onIterationTerminated(Context context, Collector<Integer> collector) {
            ...
            context.output(modelDataOutputTag, modelList);
        }

        @Override
        public void flatMap(Factors factors, Collector<Integer> collector) throws Exception {
            if (factorsMap.containsKey(String.valueOf(factors.identity) + factors.nodeId)) {
                factorsMap.get(String.valueOf(factors.identity) + factors.nodeId).factors =
                        factors.factors;
            } else {
                factorsMap.put(String.valueOf(factors.identity) + factors.nodeId, factors);
            }
        }
    }
    ...
    private static class UpdateFactorsFunc
        ...
        public void coGroup(
            ...
            DenseIntDoubleVector x = new DenseIntDoubleVector(numFactors);
            DenseIntDoubleVector buffer = new DenseIntDoubleVector(numFactors);
                            ...
                            public void flatMap(
                                ...
                                if (firstStep) {
                                    ...
                                    if (ytyLast.f1 > ytyWithEpoch.f1) {
                                        if (BLAS.norm2(new DenseIntDoubleVector(ytyLast.f0)) == 0) {
                                            collector.collect(ytyWithEpoch);
                                        } else {
                                            collector.collect(ytyLast);
                                        }
                                    } else {
                                        if (BLAS.norm2(new DenseIntDoubleVector(ytyWithEpoch.f0)) == 0) {
                                            collector.collect(ytyLast);
                                        } else {
                                            collector.collect(ytyWithEpoch);
                                        }
    ...
    public static AlsWithPs load(StreamTableEnvironment tEnv, String path) throws IOException {
