...
public class MatVecOp {
    ...
    public static void appendVectorToMatrix(
            DenseMatrix matrix, boolean trans, int index, Vector vector) {
        if (vector instanceof DenseVector) {
            ...
            if (trans) {
                ...
                for (int i = 0; i < indices.length; ++i) {
                    Preconditions.checkArgument(
                            indices[i] <= matrix.numRows(),
                            "Index %s out of matrix size %s!",
                            indices[i],
                            matrix.numRows());
                    matrixData[startIndex + indices[i]] = values[i];
                }
            }
        }
    }

    public static void appendVectorToSparseData(
            List<Integer>[] indices, List<Double>[] values, int index, SparseVector vector) {
        double[] value = vector.getValues();
        int[] key = vector.getIndices();
        for (int j = 0; j < key.length; j++) {
            Preconditions.checkArgument(
                    key[j] < indices.length,
    ...
    public static void appendVectorToSparseData(
        ...
        int[] key = vector.getIndices();
        for (int j = 0; j < key.length; j++) {
            Tuple2<List<Integer>, List<Double>> t =
                    indexHashMap.computeIfAbsent(
                            key[j], k -> Tuple2.of(new ArrayList<>(), new ArrayList<>()));
            t.f0.add(index);
            t.f1.add(value[j]);
        }
    }
