...
import org.apache.flink.ml.common.fm.BaseFmOptimizer;

import static org.apache.flink.ml.common.fm.BaseFmTrain.EPS;

/** RMSProp optimizer. */
public class RMSProp extends BaseFmOptimizer {
    private final int modelOffset;
    private final double learnRate;
    private final double gamma;

    public RMSProp(int[] dim, double learnRate, double std, double gamma) {
        super(dim, std);
    ...
    public void update(long[] keys, double[] values) {
        for (int i = 0; i < keys.length; ++i) {
            ...
            if (keys[i] == -1L) {
                ...
                sigmaGii[0] = gamma * sigmaGii[0] + (1 - gamma) * grad * grad;
                ...
                for (int j = 0; j < modelOffset; ++j) {
                    ...
                    sigmaGii[j] = gamma * sigmaGii[j] + (1 - gamma) * grad * grad;
