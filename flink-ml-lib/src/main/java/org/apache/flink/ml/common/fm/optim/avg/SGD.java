...
/**
 * An iteration stage that uses the current batch samples to update the model values with SGD
 * method.
 */
public class SGD extends BaseAvgOptimizer {
    private final LossFunction lossFunc;
    private final int[] dim;
    private final double[] regular;
    private final double learnRate;

    public SGD(LossFunction lossFunc, int[] dim, double[] regular, double learnRate) {
        super(dim, dim[1] + dim[2] + 1);
    ...
    protected double updateFactors(
        ...
        for (FmSample dataPoint : batchData) {
            ...
            if (dim[0] > 0) {
                ...
                modelValues[idx] -= learnRate * grad;

                modelValues[idx + offset] += dataPoint.weight;
            }
            ...
            for (int i = 0; i < indices.length; ++i) {
                ...
                for (int j = 0; j < dim[2]; j++) {
                    ...
                    modelValues[idx + j] -= learnRate * grad;
                }
                if (dim[1] > 0) {
                    double grad = dLdy * vals[i] + regular[1] * modelValues[idx + dim[2]];
                    modelValues[idx + dim[2]] -= learnRate * grad;
                }
            }
        }
        return loss;
