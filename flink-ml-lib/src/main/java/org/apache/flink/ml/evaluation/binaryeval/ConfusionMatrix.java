...
public class ConfusionMatrix implements Serializable {
    ...
    /**
     * PredictLabelFrequency records the frequency of each label in the prediction result. It's also
     * the sum of each row.
     */
    private final long[] actualLabelFrequency;

    /**
     * ActualLabelFrequency records the actual frequency of each label. It's also the sum of each
     * column.
     */
    private final long[] predictLabelFrequency;
    ...
    public ConfusionMatrix(LongMatrix longMatrix) {
        Preconditions.checkArgument(
                longMatrix.getRowNum() == longMatrix.getColNum(),
    ...
    double numTruePositive(Integer labelIndex) {
        Preconditions.checkArgument(
                null == labelIndex || labelIndex < labelCnt,
    ...
    double numTrueNegative(Integer labelIndex) {
        Preconditions.checkArgument(
                null == labelIndex || labelIndex < labelCnt,
                "labelIndex must be null or less than " + labelCnt);
        return null == labelIndex
                ? tnCount
                : longMatrix.getValue(labelIndex, labelIndex)
                        + total
    ...
    double numFalsePositive(Integer labelIndex) {
        ...
        return null == labelIndex
                ? fpCount
                : predictLabelFrequency[labelIndex] - longMatrix.getValue(labelIndex, labelIndex);
    }
    ...
    double numFalseNegative(Integer labelIndex) {
        ...
        return null == labelIndex
                ? fnCount
                : actualLabelFrequency[labelIndex] - longMatrix.getValue(labelIndex, labelIndex);
    }
