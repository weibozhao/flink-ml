 ...
 * labeled examples. Note that though there is no ``training process`` in KNN, we create a ``fake
 * one`` to use in pipeline model. In this operator, we do some preparation to speed up the
 * inference process.
 */
public final class KnnTrainBatchOp extends BatchOperator<KnnTrainBatchOp>
    ...
    public static Param<String> ID_TYPE = new StringParam("idType", "id type", null);
    ...
    public static DataStream<Row> buildModel(
        ...
        BaseFastDistance fastDistance =
                ((DistanceType) params.get(HasKnnDistanceType.DISTANCE_TYPE)).getFastDistance();
