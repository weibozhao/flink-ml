...
/**
 * An iteration stage that uses the current batch samples to update the model values with AdaGrad
 * method.
 */
public class AdaGrad extends BaseAvgOptimizer {
    private final LossFunction lossFunc;
    private final int[] dim;
    private final double[] regular;
    private final double learnRate;

    public AdaGrad(LossFunction lossFunc, int[] dim, double[] regular, double learnRate) {
    ...
    private void updateOneDim(double grad, double[] modelValues, int idx, int offset, int bias) {
        ...
        modelValues[idx + bias] -=
                learnRate * grad / (Math.sqrt(modelValues[idx + offset + bias] + EPS));
    }
}
