...
public class AlsModel implements Model<AlsModel>, AlsModelParams<AlsModel> {
    ...
    private static class PredictLabelFunction extends RichMapFunction<Row, Row> {
        ...
        public Map<Long, DenseIntDoubleVector> userFactors;
        public Map<Long, DenseIntDoubleVector> itemFactors;
        ...
        public Row map(Row dataPoint) {
            if (userFactors == null) {
                AlsModelData modelData =
                        (AlsModelData)
                                getRuntimeContext().getBroadcastVariable(broadcastModelKey).get(0);
                List<Tuple2<Long, float[]>> uFactors = modelData.userFactors;
                List<Tuple2<Long, float[]>> iFactors = modelData.itemFactors;
                ...
                for (Tuple2<Long, float[]> t2 : uFactors) {
                    ...
                    this.userFactors.put(t2.f0, new DenseIntDoubleVector(values));
                }
                for (Tuple2<Long, float[]> t2 : iFactors) {
                    ...
                    this.itemFactors.put(t2.f0, new DenseIntDoubleVector(values));
                }
            }
        ...
        private Row predictRating(long userId, long itemId) {
            DenseIntDoubleVector userFeat = userFactors.get(userId);
            DenseIntDoubleVector itemFeat = itemFactors.get(itemId);
