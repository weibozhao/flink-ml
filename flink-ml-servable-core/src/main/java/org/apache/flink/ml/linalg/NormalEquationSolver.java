...
public class NormalEquationSolver {
    ...
    /** A^T * b. */
    private final DenseIntDoubleVector atb;
    ...
    public NormalEquationSolver(int n) {
        ...
        this.atb = new DenseIntDoubleVector(n);
        nnls = new NNLS();
    }
     ...
     */
    public void add(DenseIntDoubleVector a, double b, double c) {
        // ata += c * a.t * a
     ...
     */
    public void solve(DenseIntDoubleVector x, boolean nonNegative) {
        if (nonNegative) {
            nnls.initialize(n);
